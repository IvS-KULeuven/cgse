...
def test_private_caching_functions(default_env):
    ...
    assert _get_cached_setup_info(0).site_id == "HOME"

    _print_cached_setup_info()

    setup = load_setup(setup_id=28, site_id="HOME", from_disk=True)

    assert setup.get_id() == '00028'
    assert 'SETUP_HOME_00028' in str(setup.get_filename())

    _add_setup_info_to_cache(setup)

    assert _get_cached_setup_info(28) is not None
    assert _get_cached_setup_info(28).site_id == 'HOME'
    assert _get_cached_setup_info(28).path.name.endswith('SETUP_HOME_00028_240123_120028.yaml')
